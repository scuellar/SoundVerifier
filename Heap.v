...
Section ParametricInVal.
  Context {val:Type}. (*Val is the type of contents in the heap*)
  
  (* Address are pairs of 'blocks' and ofsets*)
  (* Blocks represent objects in Clight *)
  Definition address:= (positive * Int.int)%type.

  (*Address is decidable*)
  ...
  (*Heap maps address to memory locations*)
  Definition heap:= address -> option val.
  Definition empty_heap:heap := fun _ => None.

  (* Heaps are equivalente if pointwise equivalent*)
